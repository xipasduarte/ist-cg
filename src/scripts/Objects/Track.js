import { Group, TorusGeometry, MeshLambertMaterial, MeshPhongMaterial, MeshBasicMaterial, Mesh, Vector3, Box3 } from 'three';

const addCheerio = (group, x, y, z) => {
	const AABB = new Box3();
	const geometry = new TorusGeometry(0.75, 0.3, 5, 20);
	const BasicMaterial = new MeshBasicMaterial({color: 0xcccc00,wireframe: true});
	const LambertMaterial = new MeshLambertMaterial({color: 0xcccc00,wireframe: true});
	const PhongMaterial = new MeshPhongMaterial({color: 0xcccc00,wireframe: true});
	const cheerio = new Mesh(geometry, PhongMaterial);
